                  ...
                  <div class="table-wrapper table-responsive">
                    <table class="table">
                      ...
                      <tr>
                          ...
                          <th class='text-center'><h6>الرقم</h6></th>
                          <th><h6>Action</h6></th>
                        </tr>
                      </thead>
                      ...
                      <?php
                        ...
                        $users = $conn -> prepare("SELECT * FROM charity_numbers");
                        $users -> execute();
                        $rows= $users -> fetchAll();
      ...
      ?>
        <tr>
          ...
          <td class="min-width text-center">
           <a href="?action=show&id=<?= $row['id'] ?>"><p><?php echo $row['name'] ?></p></a>  
          </td>
...
// #######################################################################
    ...
    @$id = $_GET['id'];
        ...
        $stmt = $conn -> prepare("SELECT * FROM charity_numbers WHERE id = ?");
...
//######################################################################           
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST' ){
        ...
        $stmt = $conn -> prepare ("UPDATE charity_numbers SET name = ? , number =?  WHERE id = ?");
        $stmt -> execute(array($name , $number , $id ));
              ...
              <div class="card-style settings-card-2 mb-30">
                  <div class="row">
                    ...
                    <div class="col-12">
                      <div class="input-style-1">
                        <label>الرقم</label>
                        <input name='number' type="text" placeholder="الرقم"/>
                      </div>
                    </div>
                    <div class="col-12">
...
//  ##################################################################################
     ...
     foreach($formErrors as $error){
      ...
      if(empty($formErrors)){
        ...
        $stmt = $conn -> prepare("INSERT INTO charity_numbers (name , number) VALUES (:name , :number)");
          $stmt -> execute(array(
          'name'   => $name,
          'number'   => $number,
          ));
          echo "<div class='alert alert-success text-center container'>" . $stmt->rowCount() . " Recored Done</din>";
          header("refresh:2;url=numbers.php");
          }
...
//#####################################################################################
  ...
  $stmt_delete = $conn -> prepare("SELECT * FROM charity_numbers WHERE id = ?");
  $stmt_delete -> execute(array($id));
  $count_delete = $stmt_delete -> rowCount();

if($count_delete > 0){

    $stmt = $conn -> prepare("DELETE FROM charity_numbers WHERE id = ?");
    $stmt -> execute(array($id));
