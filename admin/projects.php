     ...
     ?>
      ...
      <section class="card-components">
        <div class="container-fluid">
          ...
          <?php
if($action == 'manage'){
    $cats = $conn -> prepare("SELECT * FROM projects");
    $cats -> execute();
    $rows= $cats -> fetchAll();
              ...
              <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6">
                <div class="card-style-1 mb-30">
                  <div class="card-image">
                    <a href="projects_pro.php?proid=<?php echo $row['id'] ?>">
            ...
            <div class="card-style settings-card-2 mb-30">
              <div class="title mb-30">
                ...
                <div class="row">
                  ...
                  <div class="col-12">
                    <div class="input-style-1">
                      <label>معلومات عن المشروع</label>
...
// #############################################################################
              ...
              if($_SERVER['REQUEST_METHOD'] == 'POST'){
                    ...
                    $formErrors = [];
                    if(empty($name)){
                    $formErrors[] = "الرجاء ملئ اسم المشروع";
                    }
                    if(empty($info)){
                      $formErrors[] = "الرجاء اضافة معلومات عن المشروع ";
                    }


                  foreach($formErrors as $error){
                        echo "<div class='alert alert-danger container text-center'>" .$error. "</div>";
                        header("refresh:2;url=projects.php?action=add");
    ...
    <div class="col-lg-10" style='margin: 50px ;'>
               <div class="card-style settings-card-2 mb-30">
                 <div class="title mb-30">
                   <h6>تعديل علي البمشروع</h6>
                 </div>
                   <div class="row">
                   <div class="col-12">
                    <div class="input-style-1">
                      <label> عنوان المشروع </label>
                      <input name='name' type="text" placeholder="اسم المشروع" value='<?= $row['name'] ?>'/>
                    </div>
                    </div>
                     </div>
                     <div class="col-12">
...
// #########################################################################################  
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST'){
                ...
                foreach($formErrors as $error){
                   ...
                   $stmt = $conn -> prepare("SELECT * FROM projects WHERE name= ? AND id != ?");
                    $stmt ->execute(array($name , $id));
                    $count = $stmt -> rowCount();

                  if($count == 1){

                  echo "<div class='alert alert-danger text-center container'>هذا المشروع موجود بالفعل</div>";
                  header("refresh:2;url=".$_SERVER['HTTP_REFERER']."");
                  exit();

              }else{

                  $stmt = $conn -> prepare("UPDATE projects SET name = ? , about = ? , image = ? WHERE id = ?");
   ...
   // #####################################################################################   
    ...
    $stmt_delete = $conn -> prepare("SELECT * FROM projects WHERE id = ?");
