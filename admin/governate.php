     ...
     ?>
      ...
      <section class="card-components">
        <div class="container-fluid">
          ...
          <?php
if($action == 'manage'){
    $cats = $conn -> prepare("SELECT * FROM governate");
    $cats -> execute();
    $rows= $cats -> fetchAll();
              ...
              <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6">
                <div class="card-style-1 mb-30">
                  <div class="card-image">
                    <a href="?action=edit&Govid=<?php echo $row['id'] ?>">
                    </a>
                  </div>
                  <div class="card-content text-center">
                    <h4><a href="governate_content.php?Govid=<?= $row['id'] ?>"> <?php echo $row['name'] ?> </a></h4>
                  </div>
...
// #############################################################################
              ...
              if($_SERVER['REQUEST_METHOD'] == 'POST'){
                  ...
                  foreach($formErrors as $error){
                        ...
                        header("refresh:2;url=governate.php?action=add");
          ...
          if(empty($formErrors)){
                ...
                $stmt = $conn -> prepare("SELECT * FROM governate WHERE name = ?");
    ...
    $stmt = $conn -> prepare("INSERT INTO governate
                      (name)
              VALUES
                      (:name)");
            ...
            'name'       => $name,
                  ...
                  header("refresh:2;url=governate.php");
          }
          }
...
// #########################################################################################  
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST'){
                ...
                foreach($formErrors as $error){
                   ...
                   $stmt = $conn -> prepare("SELECT * FROM governate WHERE name= ? AND id != ?");
                  ...
                  $stmt = $conn -> prepare("UPDATE governate SET name= ?  WHERE id = ?");
   ...
   // #####################################################################################   
    ...
    $stmt_delete = $conn -> prepare("SELECT * FROM governate WHERE id = ?");
    $stmt_delete -> execute(array($id));
    $count_delete = $stmt_delete -> rowCount();
  
  if($count_delete > 0){
  
      $stmt = $conn -> prepare("DELETE FROM governate WHERE id = ?");
      $stmt -> execute(array($id));
